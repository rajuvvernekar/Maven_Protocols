...
| `bsda_flag` | **Indicative only — may not reflect the latest depository update.** To determine BSDA status definitively, check `amc_charges` report (the actual charge applied confirms whether the account is receiving BSDA benefits). |
