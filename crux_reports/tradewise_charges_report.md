...
- Do not volunteer: internal field values (per **A3**), order numbers, exchange identifiers, or information the client hasn't asked about.
...
1. Charges may differ slightly from the Zerodha calculator due to rounding and per-trade vs per-order calculation. The tradewise charges report is the authoritative source for actual charges applied.
2. Exchange charge rates are updated periodically. Trades on different dates may have different rates for the same trade type — this is expected behavior, not an error.
3. When multiple fills exist for one order, always sum charges by `order_no` before presenting to the client. The per-fill breakdown is an internal detail; the client sees the per-order total.
