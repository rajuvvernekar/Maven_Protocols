...
| Buyback entry posting | Posted on the day the "Net settlement for buyback with settlement number" entry appears in `ledger_report`; escalate|
...
3. `pending_recalc` = false AND buy average still wrong in `console_eq_holdings`:
   ...
   b. Entry details are correct but buy average remains wrong → escalate. This is a backend recalculation failure.
...
2. Entry exists:
   ...
   b. If client also has a separate discrepant entry for the same stock → escalate.
...
3. Client wants actual acquisition cost instead of the default closing price → this requires a backend correction. Escalate.
...
4. Any other ESOP issue → escalate.
...
4. No entry found AND IPO listing was more than 3 trading days ago → escalate.
...
2. Shares credited via corporate action (convertible debenture, rearrangement, unclaimed shares) where the source is unclear → escalate.
...
3. No entry found → devolved entries are system-posted and should appear automatically. Escalate.
...
3. No entry found → OFS allotment entries are system-posted. Escalate.
...
3. No entry found AND client reports an unresolved short delivery → escalate. Cross-reference with `console_eq_holdings` protocol Rule 8 (Short delivery handling).
...
3. No entry found → rights issue entries are system-posted. Escalate.
...
3. No entry found AND client confirms tendering was completed → escalate.
