...
- Do not volunteer information about unrelated tradebook topics unless directly relevant.
...
- Tradebook covers the last 100 days. For older F&O trades, use `console_fno_tradebook_prepared`.
- One order may produce multiple fills — check for multiple `trade_id`s under the same `order_id`.
- Contract symbols may change mid-series due to corporate actions on the underlying. Strike and lot size adjust; position value stays the same.
- Negative quantity may represent the sell side of a trade.
- Charges, MTM calculations, and obligation breakdowns are not available in this tool — refer to the actual contract note.
- Auction trades appear with specific `order_id` patterns.
