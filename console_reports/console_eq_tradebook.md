...
If no route matches, cross-reference with **A4** tools for additional context. If no root cause is found, escalate.
...
6. If still not found after checking all sources → escalate.
...
3. If execution price materially differs from limit order price placed by client → escalate.
...
2. Escalate with: client ID, affected trade_date, order_id(s), tradingsymbol(s).
3. Escalate. The Console team handles duplicate entry corrections.
